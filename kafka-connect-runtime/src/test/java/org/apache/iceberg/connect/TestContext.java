...
public class TestContext {
  ...
  private TestContext() {
    ...
    catalog =
        new GenericContainer<>(DockerImageName.parse(REST_CATALOG_IMAGE))
            ...
            .withEnv("CATALOG_WAREHOUSE", "s3://" + TestConstants.BUCKET + "/warehouse")
            .withEnv("CATALOG_IO__IMPL", "org.apache.iceberg.aws.s3.S3FileIO")
            .withEnv("CATALOG_S3_ENDPOINT", "http://minio:9000")
            .withEnv("CATALOG_S3_ACCESS__KEY__ID", TestConstants.AWS_ACCESS_KEY)
            .withEnv("CATALOG_S3_SECRET__ACCESS__KEY", TestConstants.AWS_SECRET_KEY)
            .withEnv("CATALOG_S3_PATH__STYLE__ACCESS", "true")
            .withEnv("AWS_REGION", TestConstants.AWS_REGION);
    ...
    try (S3Client s3 = initLocalS3Client()) {
      s3.createBucket(req -> req.bucket(TestConstants.BUCKET));
    }
  ...
  public S3Client initLocalS3Client() {
    try {
      return S3Client.builder()
          ...
          .region(Region.of(TestConstants.AWS_REGION))
          .forcePathStyle(true)
          .credentialsProvider(
              StaticCredentialsProvider.create(
                  AwsBasicCredentials.create(
                      TestConstants.AWS_ACCESS_KEY, TestConstants.AWS_SECRET_KEY)))
          .build();
  ...
  public Catalog initLocalCatalog() {
    ...
    result.initialize(
        ...
        ImmutableMap.<String, String>builder()
            ...
            .put("s3.access-key-id", TestConstants.AWS_ACCESS_KEY)
            .put("s3.secret-access-key", TestConstants.AWS_SECRET_KEY)
            .put("s3.path-style-access", "true")
            .put("client.region", TestConstants.AWS_REGION)
            .build());
    return result;
  }
  ...
  public Map<String, Object> connectorCatalogProperties() {
    return ImmutableMap.<String, Object>builder()
        ...
        .put("iceberg.catalog.s3.access-key-id", TestConstants.AWS_ACCESS_KEY)
        .put("iceberg.catalog.s3.secret-access-key", TestConstants.AWS_SECRET_KEY)
        .put("iceberg.catalog.s3.path-style-access", true)
        .put("iceberg.catalog.client.region", TestConstants.AWS_REGION)
        .build();
  }
